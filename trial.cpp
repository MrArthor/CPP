...
//============================================================================================================================//
class Process
{ /// Processes class, contains more than needed functions/data// for experimental and developement
public:
    Process();
    ~Process();
    Process(int);
...
//-----------------------------------------------------------------
int find_how_many_Pages_bits(int j)
{ // j is # pages
    if (j <= 2)
    {
        return 1;
...
bool cmp(Process s1, Process s2)
{
    ...
    { // if both share the same cylinder, then use the id as breaker
...
void runningMode()
{
    ...
    if (input == "A")
    {
        ...
        p.set_ProcSize(pz);
        int FN;
        if (p.get_ProcSize() % pageSize == 0)
        {
            FN = p.get_ProcSize() / pageSize;
        }
        else if (p.get_ProcSize() % pageSize != 0)
        {
            FN = (p.get_ProcSize() / pageSize) + 1;
        }
    ...
    while (true)
    {
        ...
        else if (action == "t")
        {
            ...
            else if (!CPU_is_free)
            {
                ...
                cout << " Process ID ||" << setw(12) << " Total CPU time ||" << setw(12) << "Avg. Proc. Time|| "
                     << "AVG Burst Time " << endl;
                cout << "---------------------------------------------------------------------------" << endl;
                ...
                if (JobPool.size() > 0)
                {
                    ...
                    {
                        if (JobPool[j].get_framesNeeded() <= FreeFrames)
                        {   // you need to make it by frames
                            //----------------------------------------------------
        ...
        else if (action == "p0" || action == "p1" || action == "p2" || action == "p3" || action == "p4" || action == "p5")
        {
            ...
            else
            {
                ...
                else if (!CPU_is_free)
                {
                    ...
                    if (maxPagesPerProc < cpuQueue.front().get_pageNum())
                    { // maybe <=
        ...
        else if (action[0] == 'K')
        {
            ...
            if (readyQueue.size() > 0 && !found)
            {
                ...
                while (backup.size() > 0)
                { // reconstruct ready queue with same order - use function reorder- delete the value
                    if (backup.front().getID() == KDN)
                    { //from second queue
                ...
                if (ki > 0)
                {
                    ...
                    if (q2.size() > 0)
                    {
                        ...
                        for (int j = 0; j < freeFramesV.size(); j++)
                        {   // lazy delete it from freeframes
                            //  cout <<"++++++++++++++"<<endl;
                            //  cout <<freeFramesV[j].get_occupied_by_processId()<<endl;
        ...
        else if (action == "S")
        {
            ...
            {
                backup = readyQueue;
                cout << " PID   " << setw(12) << "|| Coming From  ||" << setw(12) << " Processing Time" << setw(12) << " ||AVG Proc.Time"
                     << "||AVG Burst Time" << endl;
                cout << "-------------------------------------------------------" << endl;
                while (backup.size() > 0)
                {
            ...
            if (show == 'p')
            {
                ...
                cout << "PID " << setw(5) << "||R/W" << setw(10) << "|| M.Start" << setw(10) << "||Length" << setw(10) << "||File Name" << setw(10) << "||Proc.Time" << setw(14) << "||AVG Proc.Time"
                     << "|| Avg Burst T" << endl;
                cout << "--------------------------------------------------------------------------------" << endl;
                for (int i = 0; i < m; i++)
                {
            ...
            if (show == 'd')
            {
                ...
                cout << "PID" << setw(7) << "|R/W" << setw(9) << "|M.strat" << setw(8) << "|Length" << setw(8) << "|F.Name" << setw(8) << "|proc.time" << setw(8) << "|AVg procT" << setw(3) << "||cyl# "
                     << "|Avg_Burst_Time" << endl;
                cout << "-------------------------------------------------------------------------------" << endl;
                for (int i = 0; i < n; i++)
                {
            ...
            if (show == 'c')
            {
                ...
                cout << "PID" << setw(8) << "|R/W" << setw(12) << "|M.Start" << setw(9) << "|M.Length" << setw(8) << "|F.Name" << setw(10) << "|Proc.Time" << setw(10) << "|AVG Proc.Time"
                     << "|Avg Burst time" << endl;
                cout << "----------------------------------------------------------------------------" << endl;
                for (int i = 0; i < k; i++)
                {
...
void Process::set_ProcessAvgBurstTime(double x, int y)
{
    if (y != 0)
        ProcessAvgBurstTime = x / y;
    else
        ProcessAvgBurstTime = 0;
}
